...
LOG="$LOG_DIR/2-packer-windows.log"
...
log "============================================================"
log "  Packer: Build Windows Server 2022 Nginx HTTPS AMI"
...
log ""
log "=== packer init ==="
cd "$REPO_ROOT/packer/windows"
packer init windows.pkr.hcl 2>&1 | tee -a "$LOG"

# ── Packer build ──────────────────────────────────────────────
log ""
log "=== packer build ==="
packer build windows.pkr.hcl 2>&1 | tee -a "$LOG"
...
echo "$AMI_ID" > "$LOG_DIR/windows-ami-id.txt"

log ""
log "=== Result ==="
log "Windows AMI ID : $AMI_ID"
log "Log saved      : $LOG"
